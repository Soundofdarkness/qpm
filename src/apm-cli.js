...
const parseOptions = function(args) {
  if (args == null) { args = []; }
  const options = yargs(args).wrap(100);
  options.usage(`\

qpm - Quarks Package Manager powered by https://atom.io
...
Run \`qpm help <command>\` to see the more details about a specific command.\
`
  );
...
const printVersions = function(args, callback) {
  ...
  return getPythonVersion(pythonVersion =>
    git.getGitVersion(function(gitVersion) {
      let versions;
      if (args.json) {
        versions = {
          apm: apmVersion,
