...
describe('apm list', function() {
  ...
  it('labels disabled packages', function() {
    ...
    return listPackages([], () => expect(console.log.argsForCall[1][0]).toContain('test-module@1.0.0 (disabled)'));
  });

  it('displays only enabled packages when --enabled is called', function() {
    const atomPackages = {
      'test-module': {
        metadata: {
          name: 'test-module',
    ...
    return listPackages(['--enabled'], function() {
      ...
      return expect(console.log.argsForCall[10][0]).toContain('git-package@1.0.0');
    });
  });
